...
class Block
{
	...
	constructor(block:Block)
	{
		...
		else
		{
			...
			// Convert the RGB values to a numeric color value (e.g., hex or decimal)
let numericColor = (r << 16) | (g << 8) | b;

// Assign the numeric color value to this.color
this.color = numericColor;
		}
		...
		const translationMatrix = new THREE.Matrix4().makeTranslation(this.dimension.width / 2, this.dimension.height / 2, this.dimension.depth / 2);
geometry.applyMatrix4(translationMatrix);

this.material = new THREE.MeshToonMaterial({
    color: this.color,
    flatShading: true // Use flat shading mode
});
	...
	place():BlockReturn
	{
		...
		let blocksToReturn: BlockReturn = {
            plane: this.workingPlane as "x" | "y" | "z", // Use a type assertion
		...
		if(overlap > 0)
		{
			...
			else
			{
			 ...
			 placedMesh = new THREE.Mesh(placedGeometry, this.material);
